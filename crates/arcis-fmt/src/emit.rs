...
fn separator(
    ...
    let sep = match (prev_tok.clone(), cur.clone()) {
        ...
        (_, RBracket) => Sep::None,
        (Bang, _) => Sep::None,
        // Postfix `!` (non-null assertion) and `?` (optional-property
        // marker) hug the identifier/expression they follow: `x!`,
        // `last?: string` — never `x !` or `last ?: string`.
        (Ident(_) | RParen | RBracket | Number(_) | String(_) | Bool(_), Bang | Question) => Sep::None,
